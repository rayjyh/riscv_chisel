...
import riscv_3stage.Instructions._
...
object SortTest extends DatapathTest {
...
trait TestUtils {
  ...
  def RU(funct3: UInt, rd: Int, rs1: Int, rs2: Int): UInt =
  ...
  val bypassTest = List(
    ...
    B(Funct3.BEQ, 1, 6, 2),  // BEQ  x1, x6, 8   # go to the BGE branch
    J(0, 3),                // JAL  x0, 12      # skip nop
    B(Funct3.BGE, 4, 1, -1), // BGE  x4, x1, -4  # go to the jump
    nop, nop,
    ...
    nop,
    S(Funct3.SW, 31, 0, 12)  // SW   x31, x0, 12  # Mem[12] <- 10
    //fin
  )
  ...
  val bubbleSort = List(
    I(Funct3.ADD, 1, 0,  5),  //      start: ADDI x1, x0,  5 # x1 <- 5        512
    nop,
    RU(Funct3.ADD, 2, 0, 0),  //   	    ADD  x2, x0, x0 # x2 <- 0   514
    nop,
    I(Funct3.ADD, 6, 1, -1), //              ADDI x6, x1, -1 # x6 <- x1 - 2 516
    nop,
    I(Funct3.ADD, 3, 6, -1),  // 	     outer: ADDI x3, x6, -1  # x3 <- x6             518
    nop,
    L(Funct3.LW, 4, 3, 0),  //       inner:  LW   x4, 0(x3)  # x4 <- Mem[3]   520
    nop,
    L(Funct3.LW, 5, 3, 1),  //               LW   x5, 1(x3)  # x5 <- Mem[4]   522
    nop,
    B(Funct3.BGE, 5, 4, 6), //              BGE  x5, x4, no_swap  # go to the no_swap  524
    nop,
    S(Funct3.SW, 4, 3, 1),  //              SW   x4, 1(x3)  # Mem[4] <- x4  526
    nop,
    S(Funct3.SW, 5, 3, 0),  //              SW   x5, 0(x3)  # Mem[3] <- x5  528
    nop,
    I(Funct3.ADD, 3, 3, -1),//      no_swap: ADDI x3, x3, -1 # x3 <- x3 - 1    530
    nop,
    B(Funct3.BGE, 3, 2, -12), //            BGE  x3, x2, inner  # go to the inner  532
    nop,
    I(Funct3.ADD, 2, 2, 1),  //              ADDI x2, x2, 1 # x2 <- x2 + 1     534
    nop,
    B(Funct3.BLT, 2, 6, -18), //            BLT  x2, x6, outer  # go to the outer 536
    nop,
    B(Funct3.BEQ, 0, 0, -2) //              BEQ  x0, x0, start  # go to the start 538
  )
  val tests = Map(
    SortTest -> bubbleSort)
